...
export const stockCirculationSchema = z.object({
  ...
  mutasiMasukData: z.number().refine(n => !isNaN(n) && n >= 0, { message: "Mutasi Masuk Data tidak boleh negatif" }),
  mutasiKeluar: z.number().refine(n => !isNaN(n) && n >= 0, { message: "Mutasi Keluar Lain tidak boleh negatif" }),
  mutasiKeluarData: z.number().refine(n => !isNaN(n) && n >= 0, { message: "Mutasi Keluar Data tidak boleh negatif" }),
