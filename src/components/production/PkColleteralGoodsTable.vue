        ...
        <table class="modern-table">
          <thead>
            <tr class="table-header-row">
              ...
              <th class="table-th amount-col additional-col">PK Additions</th>
              <th class="table-th amount-col sales-col">Sales</th>
              <th class="table-th amount-col final-col">Final PK</th>
          ...
          <tbody>
            ...
            <tr v-if="visibleEntries.length === 0" class="empty-row">
              <td colspan="7" class="empty-cell">
                <div class="empty-state">
                  ...
                  <h3 class="empty-title">No PK Collateral Data Available</h3>
                  <p class="empty-description">
                    Start by adding PK collateral data using the form above
                  </p>
                </div>
              </td>
            </tr>
            ...
            >
              ...
              <td class="table-td amount-cell additional-cell">
                <div class="additional-data">
                  <div class="additional-breakdown">
                    <span class="additional-item">
                      <span class="additional-label">PK Tambahan:</span>
                      <span class="additional-value">{{
                        formatNumber(safeNumber(item.row.pk_tambahan))
                      }}</span>
                    </span>
                    <span class="additional-item">
                      <span class="additional-label">PK Tambahan Data:</span>
                      <span class="additional-value">{{
                        formatNumber(safeNumber(item.row.pk_tambahan_data))
                      }}</span>
                    </span>
                  </div>
                </div>
                ...
                <div class="sales-data">
                  <div class="sales-breakdown">
                    <span class="sales-item">
                      <span class="sales-label">Terjual:</span>
                      <span class="sales-value">{{
                        formatNumber(safeNumber(item.row.terjual))
                      }}</span>
                    </span>
                    <span class="sales-item">
                      <span class="sales-label">Terjual Data:</span>
                      <span class="sales-value">{{
                ...
                <div class="final-pk">
                  <div class="final-breakdown">
                    <span class="final-item">
                      <span class="final-label">Jumlah PK:</span>
                      <span class="final-value">{{
                        formatNumber(safeNumber(item.row.jumlah_pk))
                      }}</span>
                    </span>
                    <span class="final-item">
                      <span class="final-label">Jumlah PK Data:</span>
                      <span class="final-value">{{
                        formatNumber(safeNumber(item.row.jumlah_pk_data))
                      }}</span>
                    </span>
                  </div>
                  <svg
...
import type { PkColleteralGoodsData } from "@/types/pk-colleteral-goods.types";
import { computed } from "vue";

interface Props {
  entries: PkColleteralGoodsData[];
  currentPage: number;
  pageSize: number;
  showIndex?: boolean;
...
const totalPK = computed(() => {
  return visibleEntries.value.reduce((sum, item) => {
    return sum + safeNumber(item.row.jumlah_pk);
  }, 0);
});

const avgPK = computed(() => {
