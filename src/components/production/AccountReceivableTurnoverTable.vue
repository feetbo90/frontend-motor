    ...
    <table class="table">
      ...
      <tbody>
        <tr v-if="visibleEntries.length === 0">
          <td :colspan="showIndex ? 9 : 6" class="empty">Belum ada data. Tambahkan data piutang dan pembiayaan di atas.
          </td>
        </tr>
        <tr v-for="(item, idx) in visibleEntries" :key="item.row.id">
          ...
          <td>{{ format(item.row.kurang_lancar) }}</td>
          ...
          <td class="actions">
            <button type="button" class="btn btn-xs btn-outline" @click="$emit('edit', item.originalIndex)"><i
                class="fas fa-pen"></i> Ubah</button>
            <button type="button" class="btn btn-xs btn-danger" @click="$emit('delete', item.originalIndex)"><i
