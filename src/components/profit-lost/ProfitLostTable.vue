      ...
      <table class="table">
        <thead>
          <tr>
            ...
            <th>Jumlah Laba / Rugi Kumulatif</th>
            <th>Penarikan Laba</th>
            <th>Jumlah Modal</th>
            <th>Aksi</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="visibleEntries.length === 0">
            <td :colspan="showIndex ? 7 : 6" class="empty">Belum ada data. Tambahkan data piutang dan pembiayaan di atas.</td>
          </tr>
          <tr v-for="(item, idx) in visibleEntries" :key="item.row.id">
            ...
            <td>{{ format(item.row.labaRugiBulan) }}</td>
            <td>{{ format(item.row.labaRugiKumulatif) }}</td>
            <td>{{ format(item.row.penarikanLaba) }}</td>
            <td>{{ format(item.row.jumlahModal) }}</td>
            <td class="actions">
