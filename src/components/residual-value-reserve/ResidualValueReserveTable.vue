    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr v-for="(item, idx) in visibleEntries" :key="item.row.id">
          ...
          <td>{{ format(item.row.macet_real) }}</td>
          <td>{{ format(item.row.surplus_devist) }}</td>
          <td>{{ format(item.row.cadangan_stock) }}</td>
          <td>{{ format(item.row.cadangan_stock_data) }}</td>
          <td class="actions">
            <button type="button" class="btn btn-xs btn-outline" @click="$emit('edit', item.row.id)"><i class="fas fa-pen"></i>
              Ubah</button>
            <button type="button" class="btn btn-xs btn-danger" @click="$emit('delete', item.row.id)"><i class="fas fa-trash"></i>
              Hapus</button>
          </td>
        </tr>
      </tbody>
