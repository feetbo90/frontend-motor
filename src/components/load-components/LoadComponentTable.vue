    ...
    <div class="table-header">
      <div class="table-stats">
        ...
        <span class="stat-item">
          ...
          Total Beban: {{ formatCurrency(totalBeban) }}
        </span>
        <span class="stat-item">
          <svg
            class="stat-icon"
          ...
          Avg Beban: {{ formatCurrency(avgBeban) }}
        </span>
      </div>
    </div>
        ...
        <table class="modern-table">
          <thead>
            <tr class="table-header-row">
              ...
              <th class="table-th amount-col salary-col">Personnel Costs</th>
              <th class="table-th amount-col operational-col">Operational Costs</th>
              <th class="table-th amount-col reserves-col">Reserves & Depreciation</th>
              <th class="table-th amount-col final-col">Total Costs</th>
          ...
          <tbody>
            ...
            <tr v-if="visibleEntries.length === 0" class="empty-row">
              <td colspan="7" class="empty-cell">
                <div class="empty-state">
                  ...
                  <h3 class="empty-title">No Load Component Data Available</h3>
                  <p class="empty-description">
                    Start by adding load component data using the form above
                  </p>
                </div>
              </td>
            </tr>
            ...
            >
              ...
              <td class="table-td amount-cell salary-cell">
                <div class="salary-data">
                  <div class="salary-breakdown">
                    <span class="salary-item">
                      <span class="salary-label">Gaji/Insentif/Bonus:</span>
                      <span class="salary-value">{{
                        formatCurrency(safeNumber(item.row.gaji))
                      }}</span>
                    </span>
                  </div>
                </div>
              </td>
              <td class="table-td amount-cell operational-cell">
                <div class="operational-data">
                  <div class="operational-breakdown">
                    <span class="operational-item">
                      <span class="operational-label">Administrasi:</span>
                      <span class="operational-value">{{
                        formatCurrency(safeNumber(item.row.admin))
                      }}</span>
                    </span>
                    <span class="operational-item">
                      <span class="operational-label">Operasional:</span>
                      <span class="operational-value">{{
                        formatCurrency(safeNumber(item.row.operasional))
                      }}</span>
                    </span>
                  </div>
                  <div class="operational-total">
                    ...
                    <span class="total-value">{{
                      formatCurrency(safeNumber(item.row.beban_umum_operasional))
                    }}</span>
                  </div>
                </div>
              </td>
              <td class="table-td amount-cell reserves-cell">
                <div class="reserves-data">
                  <div class="reserves-breakdown">
                    <span class="reserves-item">
                      <span class="reserves-label">Penyusutan:</span>
                      <span class="reserves-value">{{
                        formatCurrency(safeNumber(item.row.penyusutan_aktiva))
                      }}</span>
                    </span>
                    <span class="reserves-item">
                      <span class="reserves-label">Cad. Piutang:</span>
                      <span class="reserves-value">{{
                        formatCurrency(safeNumber(item.row.cadangan_piutang))
                      }}</span>
                    </span>
                    <span class="reserves-item">
                      <span class="reserves-label">Cad. Stock:</span>
                      <span class="reserves-value">{{
...
const totalBeban = computed(() => {
  return visibleEntries.value.reduce((sum, item) => {
    return sum + safeNumber(item.row.total);
  }, 0);
});

const avgBeban = computed(() => {
