                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRateSatu(entity.name).length === 0">
                                            <td :colspan="4" class="empty">Data Rata-rata Pembiayaan / Unit Penjualan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRateSatu(entity.name)"
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRateDua(entity.name).length === 0">
                                            <td :colspan="4" class="empty">Data Rata-rata Penjualan / Unit Penjualan
                                                tidak ditemukan.</td>
                                        </tr>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRateTiga(entity.name).length === 0">
                                            <td :colspan="4" class="empty">Data Rata-rata Penjualan / Karyawan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRateTiga(entity.name)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ formatCurrency(item.total_penjualan) }}</td>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRateEmpat(entity.name).length === 0">
                                            <td :colspan="4" class="empty">Data Rata-rata Mark up / Karyawan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRateEmpat(entity.name)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ formatCurrency(item.total_markup) }}</td>
                                            <td>{{ typeof item.jumlah_karyawan === 'string' ?
                                                Number(item.jumlah_karyawan) : (item.jumlah_karyawan ||
                                                    item.total_karyawan || 0) }}</td>
                                            <td>{{ formatCurrency(item.rate_empat || item.markup_per_karyawan || 0) }}
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRateLima(entity.name).length === 0">
                                            <td :colspan="4" class="empty">Data Rata-rata Gaji / Karyawan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRateLima(entity.name)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ formatCurrency(item.gaji || item.total_gaji || 0) }}</td>
                                            <td>{{ item.jumlah_karyawan || item.total_karyawan || 0 }}</td>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRateEnam(entity.name).length === 0">
                                            <td :colspan="4" class="empty">Data Rata-rata Biaya Operasional / Karyawan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRateEnam(entity.name)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ formatCurrency(item.beban_umum_operasional ||
                                                item.total_beban_umum_operasional || 0) }}</td>
                                            <td>{{ item.jumlah_karyawan || item.total_karyawan || 0 }}</td>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRateTujuh(entity.name).length === 0">
                                            <td :colspan="4" class="empty">Data Rata-rata Beban Tetap / Karyawan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRateTujuh(entity.name)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ formatCurrency(item.penyusutan || item.total_penyusutan_aktiva || 0)
                                                }}</td>
                                            <td>{{ item.jumlah_karyawan || item.total_karyawan || 0 }}</td>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRateDelapan(entity.name).length === 0">
                                            <td :colspan="4" class="empty">Data Rata-rata Beban Tetap / Satuan Kerja
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRateDelapan(entity.name)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ formatCurrency(item.total_penyusutan_aktiva) }}</td>
                                            <td>{{ formatCurrency(item.total_unit) }}</td>
                                            <td>{{ formatCurrency(item.rate_penyusutan_aktiva_per_unit) }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRateSembilan(entity.name).length === 0">
                                            <td :colspan="6" class="empty">Data Rata-rata Beban PH dan Penyusutan /
                                                Satuan Kerja
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRateSembilan(entity.name)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ formatCurrency(item.total_penyusutan_aktiva) }}</td>
                                            <td>{{ formatCurrency(item.total_cadangan_piutang) }}</td>
                                            <td>{{ formatCurrency(item.total_cadangan_stock) }}</td>
                                            <td>{{ formatCurrency(item.total_unit) }}</td>
                                            <td>{{ formatCurrency(item.rate_penyusutan_dan_cadangan_per_unit) }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRateSepuluh(entity.name).length === 0">
                                            <td :colspan="4" class="empty">Data Rata-rata Laba/rugi nett / Satuan Kerja
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRateSepuluh(entity.name)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ formatCurrency(item.total_kumulatif) }}</td>
                                            <td>{{ formatCurrency(item.total_unit) }}</td>
                                            <td>{{ formatCurrency(item.kumulatif_per_unit) }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRateSebelas(entity.name).length === 0">
                                            <td :colspan="4" class="empty">Data Rata-rata Laba/rugi nett / Karyawan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRateSebelas(entity.name)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ formatCurrency(item.total_kumulatif) }}</td>
                                            <td>{{ formatCurrency(item.total_karyawan) }}</td>
                                            <td>{{ formatCurrency(item.kumulatif_per_karyawan) }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
            ...
            <div v-for="entity in apiRatioData.entityIds" :key="entity.id" class="entity-card">
                ...
                <transition name="slide-fade">
                    <div v-show="!collapsed[`ratio-${entity.id}`]" class="entity-content">
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRasioSatu(entity.name, entity.type).length === 0">
                                            <td :colspan="4" class="empty">Data Ratio Pembiayaan / Realisasi Pokok
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRasioSatu(entity.name, entity.type)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ formatCurrency(item.pembiayaan) }}</td>
                                            <td>{{ formatCurrency(item.realisasi_pokok) }}</td>
                                            <td>{{ item.pembiayaan_per_realisasi_pokok }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRasioDua(entity.name, entity.type).length === 0">
                                            <td :colspan="5" class="empty">Data Ratio Kn /T Kemacetan / Pembiayaan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRasioDua(entity.name, entity.type)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ item.macet_lama }}</td>
                                            <td>{{ item.stock_kredit }}</td>
                                            <td>{{ item.leasing }}</td>
                                            <td>{{ item.rasio_kemacetan_pembiayaan }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRasioTiga(entity.name, entity.type).length === 0">
                                            <td :colspan="4" class="empty">Data Ratio Mark up / Pembiayaan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRasioTiga(entity.name, entity.type)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ item.total_markup }}</td>
                                            <td>{{ item.pembiayaan }}</td>
                                            <td>{{ item.rasio_markup }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRasioEmpat(entity.name, entity.type).length === 0">
                                            <td :colspan="4" class="empty">Data Ratio Pendapatan Bunga / So. Piutang
                                                Akhir
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRasioEmpat(entity.name, entity.type)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ item.realisasi_bunga }}</td>
                                            <td>{{ item.total }}</td>
                                            <td>{{ item.rasio_realisasi_bunga_per_total_piutang }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRasioLima(entity.name, entity.type).length === 0">
                                            <td :colspan="4" class="empty">Data Ratio Mark up / Jumlah Pendapatan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRasioLima(entity.name, entity.type)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ item.jumlah_pendapatan }}</td>
                                            <td>{{ item.total_markup }}</td>
                                            <td>{{ item.rasio_markup_per_jumlah_pendapatan }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRasioEnam(entity.name, entity.type).length === 0">
                                            <td :colspan="4" class="empty">Data Ratio Pendapatan Bunga / Jumlah
                                                Pendapatan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRasioEnam(entity.name, entity.type)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ item.jumlah_pendapatan }}</td>
                                            <td>{{ item.realisasi_bunga }}</td>
                                            <td>{{ item.rasio_pendapatan_bunga_per_jumlah_pendapatan }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRasioTujuh(entity.name, entity.type).length === 0">
                                            <td :colspan="5" class="empty">Data Ratio Pendapatan Lainnya / Jumlah
                                                Pendapatan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRasioTujuh(entity.name, entity.type)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ item.jumlah_pendapatan }}</td>
                                            <td>{{ item.denda }}</td>
                                            <td>{{ item.administrasi }}</td>
                                            <td>{{ item.rasio_pendapatan_lainnya_per_jumlah_pendapatan }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRasioDelapan(entity.name, entity.type).length === 0">
                                            <td :colspan="4" class="empty">Data Ratio Gaji / Pendapatan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRasioDelapan(entity.name, entity.type)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ item.jumlah_pendapatan }}</td>
                                            <td>{{ formatCurrency(item.gaji) }}</td>
                                            <td>{{ item.rasio_gaji_per_pendapatan }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRasioSembilan(entity.name, entity.type).length === 0">
                                            <td :colspan="4" class="empty">Data Ratio Biaya Admin dan operasi /
                                                Pendapatan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRasioSembilan(entity.name, entity.type)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ item.jumlah_pendapatan }}</td>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <tr v-if="getRasioSepuluh(entity.name, entity.type).length === 0">
                                            <td :colspan="4" class="empty">Data Ratio Biaya Tetap (Penyusutan) /
                                                Pendapatan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRasioSepuluh(entity.name, entity.type)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ item.jumlah_pendapatan }}</td>
                        ...
                        <div class="metric-card">
                            <div class="card-header yellow">Ratio Biaya Tidak Tetap (Ph.p/stock) / Pendapatan</div>
                            <div class="card-body">
                                <table class="table">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        <tr v-if="getRasioSebelas(entity.name, entity.type).length === 0">
                                            <td :colspan="4" class="empty">Data Ratio Biaya Tidak Tetap (Ph.p/stock) /
                                                Pendapatan
                                                tidak ditemukan.</td>
                                        </tr>
                                        <tr v-for="item in getRasioSebelas(entity.name, entity.type)"
                                            :key="`${item.year}-${item.month}`">
                                            <td>{{ item.month }}/{{ item.year }}</td>
                                            <td>{{ item.jumlah_pendapatan }}</td>
